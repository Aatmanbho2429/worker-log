 ...
 *
 * Position rather than id, so the two grades the app ships with keep the blue
 ...
 *
 * The classes themselves live in `styles/components/_grade-tone.scss`; each one
...
export function gradeToneClass(index: number): string {
  ...
  const n = ((index % TONES) + TONES) % TONES;
  return `grade-tone grade-tone--${n}`;
}

...
/** Shared with `send-otp`, which hashed the code this recomputes. */
const PEPPER = Deno.env.get('OTP_PEPPER') ?? '';

/**
 * Wrong guesses allowed against one code before it is destroyed.
 *
 * This, not the length of the code, is what makes four digits safe: five
 * guesses out of 10,000 is a one-in-two-thousand chance of getting in, and
 * the sixth guess deletes the row and forces a fresh send — which `send-otp`
 * then rate-limits to five an hour.
 */
...
/** Must match `send-otp` exactly, or every code looks wrong. */
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    // ------------------------------------------------- is the code right? ---
    //
    // Last of the three refusals, and still before anything is created, so a
    // wrong code leaves nothing behind either.
    //
    // Not form validation: the code is a credential, not a field the register
    // form has rules about. It was mailed by `send-otp` and is checked here
    // because here is the only place holding the hash to check it against.
    const otpCode = String(body.otpCode ?? '').trim();
    if (!otpCode) {
      return fail('badRequest', 'Enter the code sent to your email address.');
    }

    const { data: pending, error: otpError } = await admin
      .from('email_otps')
      .select('*')
      .eq('email', email)
      .maybeSingle();
    ...
    // Each dead end deletes the row rather than leaving it to be retried:
    // an expired or exhausted code has to be replaced by a fresh send, and
    // `send-otp` is what rate-limits those.
    ...
    // `email_confirm: true` because the address has just been proved by the
    // code above — a confirmation mail would ask the operator to prove the
    // same thing twice, and they have to be able to use the machine straight
    // away. `auth_register` signs them in the moment this returns, which a
    // pending confirmation would block.
    ...
    // The code has done its job. Deleted only now, after the account is fully
    // written: had it gone earlier, a failed insert would roll the auth user
    // back and leave the operator holding a code that no longer works, unable
    // to retry without waiting out the send cooldown.
    await admin.from('email_otps').delete().eq('email', email);

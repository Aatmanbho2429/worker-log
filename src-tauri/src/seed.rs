...
//! rather than through the normal add path, because the point is to fill the
//! *previous* month as well as this one — that is what gives the date
//! filters, the "Last month" preset and the PDF export something to show.
//!
//! Reached from the app's Settings screen via the `seed_demo_data` command.
//! It refuses to touch a database that already holds workers unless `force`
//! is set.
...
/// Returns a one-line summary the caller can show to the operator.
pub fn run(connection: &mut Connection, force: bool) -> AppResult<String> {
    let existing: i64 =
    ...
    if force {
        ...
        log::info!("cleared existing waste log, workers and series");
    }
